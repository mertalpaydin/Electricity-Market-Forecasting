...
def smoothed_smape(y_true: torch.Tensor, y_pred: torch.Tensor, eps: float = 1e-6) -> torch.Tensor:
    """
...
class ElectricityDataModule(pl.LightningDataModule):
    ...
    def compute_validation_metrics(self, preds_tensor: torch.Tensor, targets_tensor: torch.Tensor, asset_ids: List[str]) -> float:
        """
        ...
        original scale and then computes the sMAPE.
        ...
        preds_inv_t = torch.from_numpy(preds_inv).to(preds_tensor.device)
        targets_inv_t = torch.from_numpy(targets_inv).to(targets_tensor.device)

        smape = smoothed_smape(targets_inv_t, preds_inv_t)
        return smape.item()